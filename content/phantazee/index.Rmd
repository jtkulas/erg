...
qb <- read.csv("qb.csv", header=FALSE)
wr <- read.csv("wr.csv", header=FALSE)
rb <- read.csv("rb.csv", header=FALSE)
...
## Top 3 per position (QB, RB, WR)

```{r subset, echo=FALSE, warning=FALSE, message=FALSE}

newdata <- all[ which(all$leader < 4), ]

```

Smoothed averages by position (top 3 per week):
...
```{r box2, echo=FALSE, warning=FALSE, message=FALSE, fig.cap="Boxplot estimates show variability within groups of top 3s."}
...
Individual performances (top 3 per position):

```{r jitter2, echo=FALSE, warning=FALSE, message=FALSE, fig.cap="Jittered points top 3 performers (qb, wr, rb)."}

## getting jitter
newdata$jitter <- runif(144, min = -.5, max = .5) 
