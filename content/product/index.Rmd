...
together <- rbind(pilot, qualtrics1, qualtrics2, prolific2, snowball)

library(leaflet)
library(maps)  ## us.cities data

together$`LocationLongitude` <- round(as.numeric(as.character(together$`LocationLongitude`)),1)
...
leaflet() %>% 
  ...
  addTiles() %>%
  addMarkers(data=together, ~LocationLongitude, ~LocationLatitude, icon = ergIcon,  clusterOptions = markerClusterOptions())
