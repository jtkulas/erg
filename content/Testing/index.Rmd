...
The above map contains information from `r nrow(states)` stations in MN, WI, SD, and IA. `tidygeocoder` was unable to generation longitude and latitude estimates from `r nrow(missing)` stations. These `r nrow(missing)` stations are located in the following table:

```{r missingstations, warning=FALSE, echo=FALSE}
library(DT)
datatable(missing, options = list(pageLength = 5))
```
