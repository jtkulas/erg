...
states$octane[states$max > 91 & states$max < 100] <- "91 - 99"
states$octane[states$max > 100] <- "Greater than 100"

states$octane <- factor(states$octane, levels = c("less than 91", "91", "91 - 99", "Greater than 100"))

states$mapping <- as.numeric(as.character(str_sub(states$Octane, start= -1)))

pal2 <- colorFactor(
  palette = c("#FF0000", "#FFFF00", "#FF9933", "#FF00CC"),
...
altmap <- leaflet() %>% 
  ...
  addCircles(data=states, lng = ~long, lat = ~lat, weight = 1,
             radius = ~max*10, popup = ~paste(Name, "<br>",Street.Address, "<br>", City, State, "<br>", Phone, "<br>", "Max Octane:", max), fillColor=~pal2(octane)) %>%
